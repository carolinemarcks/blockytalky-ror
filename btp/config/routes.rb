Btp::Application.routes.draw do
    ...
    get '/static/:name.:ext' => redirect {|params, request| "/assets/static/#{params[:name]}.#{params[:ext]}?#{request.query_string}" }, constraints: { name: /.+/, ext: /(js|html|png|css|mp3|cur|gif|wav|ogg)/ }
    get '/media/:name.:ext' => redirect {|params, request| "/assets/static/media/#{params[:name]}.#{params[:ext]}?#{request.query_string}" }, constraints: { name: /.+/, ext: /(js|html|png|css|mp3|cur|gif|wav|ogg)/ }
